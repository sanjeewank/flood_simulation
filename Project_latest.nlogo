...
to setup
  ...
  clear-all-plots
  ask patches
    [ set pcolor scale-color white elevation color-min color-max ]
  set border patches with [ count neighbors != 8 ]
  reset-ticks
  ask patches [set water-level 0]
  set filled-rate 0
  set damaged-houses 0
  set total-gound-patches count patches
  set affected-patches 0
  set displacement 0
  set alert? false

  create-houses nb_individuals / 4[
    set shape "house"
    set color green
    let location one-of patches with  [elevation <= 250 and elevation > 130]
    move-to location
  ]
  ask houses [
    set size size * 8
  ...
  create-individuals nb_individuals[
    set homeplace one-of houses
    set workplace one-of offices
    set shape "person"
    set color yellow
    set age random 90;
    move-to homeplace
...
to evacuate
  ask individuals [
    ifelse pcolor = blue [
      let target min-one-of patches with [pcolor != blue] [distance myself]
      face target
      if(age >= 70)[forward 0.5]
      if(age >= 40)[forward 1]
      if(age >= 20)[forward 2]
      if(age >= 0)[forward 0.5]
      set color red
    ][
      let target min-one-of patches with [elevation >= 400 ] [distance myself]
      face target
      if(age >= 70)[forward 0.5]
      if(age >= 40)[forward 1]
      if(age >= 20)[forward 2]
    ...
    let min_elev_neighbor min-one-of neighbors [elevation]
    if water-level >= 5 [
  ...
  set displacement count individuals with [Homeplace != nobody and [color] of Homeplace = red]
  set affected-patches count patches with [pcolor = blue]
  set damaged-houses count houses with [color = red]
end
...
to spread-flood
  if(filled-rate < rain_duration + 1)[
    if(water-places != 0)[
      ask water-places[
      set water-level water-level + 0.5
      ]
    ]
  set filled-rate filled-rate + severity-rate / 200
  set water-places patches with [pcolor != blue and elevation < rain-rate * filled-rate]
  ask water-places [
      set pcolor blue set water-level water-level + 0.01
    ]
  ]
  set water-places patches with [pcolor = blue]
end
...
1827
717
Percentage of Affected Ground Area 
affected-patches / total-gound-patches * 100
2
1
14
...
1574
718
Percentage of Dead People
deadcount / nb_individuals * 100
2
1
14
...
593
366
Damaged Houses and Ground Area
NIL
Percentage
0.0
10.0
...
"" ""
PENS
"Percentage of Damaged Houses " 1.0 0 -13345367 true "" "plot damaged-houses /(nb_individuals / 4) * 100"
"Percentage of Afected Ground Area" 1.0 0 -16383231 true "" "plot affected-patches / total-gound-patches * 100"
...
"" ""
PENS
"Percentage of Displacement People" 1.0 0 -16777216 true "" "plot displacement / nb_individuals * 100"
"Percentage of Dead People" 1.0 0 -13345367 true "" "plot deadcount / nb_individuals * 100"
...
National Elevation Dataset: https://lta.cr.usgs.gov/NED
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <metric>displacement + deadcount</metric>
    <metric>damaged-houses</metric>
    <metric>individualscount</metric>
    <metric>deadcount</metric>
    <metric>deadcount / nb_individuals * 100</metric>
    <metric>affected-patches / total-gound-patches * 100</metric>
    <enumeratedValueSet variable="rain-rate">
      <value value="40"/>
      <value value="45"/>
      <value value="50"/>
      <value value="60"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rain_duration">
      <value value="1"/>
      <value value="3"/>
      <value value="5"/>
      <value value="7"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="nb_individuals">
      <value value="1000"/>
    </enumeratedValueSet>
